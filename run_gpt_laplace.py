...
def main(load_step):
    ...
    root_default_dir = "/root/.cache/huggingface/metrics/accuracy/default/" + laplace_output_dir
    print(root_default_dir)
    ...
    peft_config = PeftConfig.from_pretrained("./outputs/winogrande_s/NousResearch/Llama-2-7b-hf_lora_16_0.1_5e-05_0/step_0/")
    model = AutoModelForCausalLM.from_pretrained(
        peft_config.base_model_name_or_path, load_in_8bit=True
    )
    model = PeftModel.from_pretrained(model, "./outputs/winogrande_s/NousResearch/Llama-2-7b-hf_lora_16_0.1_5e-05_0/step_0")
    ...
    class CustomLMHead_lora(torch.nn.Module):
        def __init__(self, original_lm_head, id_list):
            ...
            self.lora_dropout = torch.nn.Dropout(p=0.1, inplace=False)
            ...
            self.lora_A = torch.nn.Linear(in_features=4096, out_features=8, bias=False).to(accelerator.device)
            torch.nn.init.xavier_uniform_(self.lora_A.weight)  # Random initialization with Xavier
            self.lora_A.weight.requires_grad = True
            ...
            self.lora_B = torch.nn.Linear(in_features=8, out_features=len(id_list), bias=False).to(accelerator.device)
            torch.nn.init.xavier_uniform_(self.lora_B.weight)  # Random initialization with Xavier
            self.lora_B.weight.requires_grad = True
    ...
    class WrappedModel(torch.nn.Module):
        def __init__(self, model):
            ...
            if args.lm_head:
                ...
                print("Original lm head before")
                print(original_lm_head)
                model.base_model.model.lm_head = CustomLMHead_lora(original_lm_head, self.id_list).to(accelerator.device) 
                print("Original lm head after")
                print(model.base_model.model.lm_head)
